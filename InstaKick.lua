...
frame.Size = UDim2.new(0, 190, 0, 275) -- Original Size
...
scroll.Size = UDim2.new(1, 0, 1, -35); scroll.Position = UDim2.new(0, 0, 0, 30)
scroll.BackgroundTransparency = 1; scroll.BorderSizePixel = 0; scroll.ScrollBarThickness = 2
scroll.CanvasSize = UDim2.new(0, 0, 0, 480) -- Content Height
...
minBox.Visible = config.minimized; minBox.Size = UDim2.new(0, 45, 0, 45); minBox.Position = frame.Position; minBox.BackgroundColor3 = Color3.fromRGB(15,15,15); minBox.Text = "NN"; minBox.Font = Enum.Font.GothamBold; minBox.TextColor3 = Color3.new(1,1,1); minBox.Draggable = true; Instance.new("UICorner", minBox).CornerRadius = UDim.new(0, 10)
...
local function createToggle(prop, text)
    ...
    btn.Size = UDim2.new(0.85, 0, 0, 32); btn.BackgroundColor3 = Color3.fromRGB(25,25,25); btn.Text = text .. ": " .. (config[prop] and "ON" or "OFF"); btn.Font = Enum.Font.GothamBold; btn.TextSize = 10; btn.TextColor3 = Color3.new(1,1,1)
...
sliderFrame.Size = UDim2.new(0.85, 0, 0, 10); sliderFrame.BackgroundColor3 = Color3.fromRGB(40,40,40); sliderFrame.BorderSizePixel = 0; Instance.new("UICorner", sliderFrame)
...
local speedValueLabel = Instance.new("TextLabel", scroll)
speedValueLabel.Size = UDim2.new(0.85, 0, 0, 15); speedValueLabel.BackgroundTransparency = 1; speedValueLabel.Text = "Speed: "..config.speed.." (Safe: 32)"; speedValueLabel.Font = Enum.Font.GothamMedium; speedValueLabel.TextSize = 9; speedValueLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
...
local function createUtil(name)
    local b = Instance.new("TextButton", scroll); b.Size = UDim2.new(0.85, 0, 0, 32); b.BackgroundColor3 = Color3.fromRGB(25,25,25); b.Text = name; b.Font = Enum.Font.GothamBold; b.TextSize = 10; b.TextColor3 = Color3.new(1,1,1)
    ...
    if config.fpsBoost then
        ...
        for _, plr in pairs(game.Players:GetPlayers()) do
            if plr ~= player and plr.Character then
                for _, acc in pairs(plr.Character:GetChildren()) do if acc:IsA("Accessory") then acc:Destroy() end end
            end
        end
    end
end)
...
RunService.Stepped:Connect(function()
    ...
    bStr.Color = color; sStr.Color = color; eStr.Color = color; rStr.Color = color; kStr.Color = color; fStr.Color = color; resS.Color = color; rejS.Color = color; leaS.Color = color
    ...
    if hum and root then
        ...
        if config.antiRagdoll then
            hum:SetStateEnabled(Enum.HumanoidStateType.FallingDown, false); hum:SetStateEnabled(Enum.HumanoidStateType.Ragdoll, false)
        end
